...
- **Roster scans are all or nothing.** A partial Lodestone list (non-200, redirect, count ≠ total) or an unreadable state message aborts before anything is posted or saved — a partial list would report members as having left. Only the bot edits the state message; an edit re-uploads every FC's file. Reports use `AllowedMentions.None` and escape names and rank names.
