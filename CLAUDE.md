...
- **Shared configuration.** The branch table and the presence timers, with a revision, live as one bot-authored message (marker + `gilgamesh-config.json`, **no token**) in every distinct state channel, found among the newest 100 messages (not pinned). "Publish to Discord" (Status tab, confirmed) writes revision `max(local, remote) + 1` to all of them; it is refused on an incomplete branch, on any branch whose server / relay / state / roster channel does not resolve with the live client, without *Attach Files* in a state channel, and when any channel already holds a newer revision. Every connected plugin reads all state channels on Ready and every 5 minutes, and applies a newer revision on the framework thread (branches + timers replaced; personal toggles kept), then reconnects if its active branch disappeared or its IDs or tag changed. Setup codes carry the revision as optional `rev`; an import sets it (0 when absent) so the next sync heals an old code.
