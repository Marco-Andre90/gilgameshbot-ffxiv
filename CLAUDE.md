...
- **FC roster.** A manual scan (settings tab or `/fcscan <world>`, any connected instance) reads the FC's Lodestone member pages, diffs them against the previous scan and posts joined / left / time in the configured starter rank to the roster channel. State lives in Discord, not in the config: one bot-authored state message in the roster channel (first message, or pinned when the channel was not empty) carrying one `roster-<lodestoneId>.json` per FC, keyed by Lodestone character id. `/fcscan` is registered identically in every guild; the worlds come from autocomplete, never from registered choices, so differing configs never re-register.
