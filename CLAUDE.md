...
- **Setup code.** A `GB2:` string (base64url JSON) carrying the token and every branch, handed over by Discord DM (`/gilga send`, receipt on import, 24 h expiry) or the clipboard, so one member configures all the others.
